...
def style_toolbar():
    """Apply Everforest styling to matplotlib toolbar"""
    try:
        import matplotlib.pyplot as plt
        ...
        if hasattr(mngr, 'toolbar'):
            toolbar = mngr.toolbar
            if hasattr(toolbar, 'setStyleSheet'):
                # Qt-specific styling
                toolbar.setStyleSheet(f"""
                    QToolBar {{
                        background-color: {EVERFOREST_COLORS["bg1"]};
                        color: {EVERFOREST_COLORS["fg"]};
                        border: 1px solid {EVERFOREST_COLORS["bg3"]};
                        spacing: 2px;
                        padding: 2px;
                    }}
                    QToolButton {{
                        background-color: {EVERFOREST_COLORS["bg2"]};
                        color: {EVERFOREST_COLORS["fg"]};
                        border: 1px solid {EVERFOREST_COLORS["bg3"]};
                        border-radius: 3px;
                        padding: 2px;
                    ...
                    QToolButton:hover {{
                        background-color: {EVERFOREST_COLORS["bg3"]};
                        color: {EVERFOREST_COLORS["fg"]};
                    }}
                    QToolButton:pressed {{
                        background-color: {EVERFOREST_COLORS["bg4"]};
                        color: {EVERFOREST_COLORS["fg"]};
                    }}
                """)
    except Exception:
        pass  # Fail silently if styling doesn't work
...
def freedman_diaconis_bins(series):
    range_len = series.max() - series.min()
    iqr = series.quantile(0.75) - series.quantile(0.25)
    n = len(series)
    h = 2 * iqr * n ** (-1/3)

    return int(range_len / h)


def create_histogram(df, title=None, category_col=None):
    """Create histogram for 2-column data (ID, Value)"""
    setup_everforest_theme()

    if len(df.columns) != 2:
        raise ValueError(
            f"Expected 2 columns for histogram, got {len(df.columns)}"
        )

    _, value_col = df.columns
    ...
    num_bins = freedman_diaconis_bins(df[value_col])
    
    if category_col:
        cats = df[category_col].unique()
        if len(cats) > 15:
            raise ValueError("chart can draw at most 15 categories")

        for i, cat in enumerate(cats):
            n, bins, patches = ax.hist(
                df[df[category_col] == cat][value_col],
                bins=num_bins,
                color=list(EVERFOREST_COLORS.values())[i % len(EVERFOREST_COLORS)],
    ...
    ax.text(
        ...
        bbox=dict(
            boxstyle="round", facecolor=EVERFOREST_COLORS["bg2"], alpha=0.8
        ),
        color=EVERFOREST_COLORS["fg"],
    )
...
def create_hexbin_plot(df, title=None, gridsize=20):
    ...
    if len(df.columns) != 3:
        raise ValueError(
            f"Expected 3 columns for hexbin plot, got {len(df.columns)}"
        )
    ...
    for col in [x_col, y_col]:
        if not pd.api.types.is_numeric_dtype(df[col]):
            try:
                ...
                raise ValueError(
                    f"Column '{col}' must be numeric for hexbin plot"
                )
    ...
    cb.set_label(
        "Point Density", fontweight="bold", color=EVERFOREST_COLORS["fg"]
    )
...
def create_scatter_plot(df, title=None):
    ...
    if len(df.columns) != 3:
        raise ValueError(
            f"Expected 3 columns for scatter plot, got {len(df.columns)}"
        )
    ...
    for col in [x_col, y_col]:
        if not pd.api.types.is_numeric_dtype(df[col]):
            try:
                ...
                raise ValueError(
                    f"Column '{col}' must be numeric for scatter plot"
                )
...
@click.option(
    ...
    "-o",
    type=click.Path(),
    help="Output file path (default: display chart)",
)
@click.option(
    "--chart-type",
    "-c",
    type=click.Choice(["auto", "histogram", "scatter", "hexbin"]),
    default="auto",
    help="Force specific chart type (default: auto-detect)",
)
@click.option(
    "--gridsize", default=20, help="Grid size for hexbin charts (default: 20)"
)
@click.option("--dpi", default=100, help="DPI for saved image (default: 100)")
@click.option(
    "--viewer",
    is_flag=True,
    help="Save to temp file and open in image viewer instead of matplotlib window",
)
def main(file, title, output, chart_type, gridsize, dpi, viewer):
    ...
    try:
        ...
        if chart_type == "auto":
            if len(df.columns) == 2:
                ...
                raise click.ClickException(
                    f"Auto-detection failed: {len(df.columns)} columns. "
        ...
        if chart_type == "histogram" and len(df.columns) != 2:
            raise click.ClickException(
                f"Histogram requires 2 columns, got {len(df.columns)}"
            )
        elif chart_type in ["scatter", "hexbin"] and len(df.columns) != 3:
            raise click.ClickException(
                f"{chart_type.title()} requires 3 columns, got {len(df.columns)}"
            )

        # Create the appropriate chart
        if chart_type == "histogram":
            click.echo("Creating histogram", err=True)
            fig = create_histogram(df, title, category_col=None)
        ...
        if output:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                for viewer_cmd in viewers:
                    ...
                    click.echo(
                        f"Could not find image viewer. Chart saved to {tmp.name}",
                        err=True,
                    )
        else:
